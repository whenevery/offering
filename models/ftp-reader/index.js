...
function upload(file , options){
    ...
    useLog.log('ftp upload is start : '+ ftpPath);
    makeDir(ftpPath,ftp,function(e){
        if(!e){
            useLog.log('read start');
            fs.readFile(file.path, function (err, data) {
                if(!err){
                    useLog.log('put start');
                    ftp.put(data , ftpPath , function (putError) {
                        useLog.log('ftp upload is :' + !putError + ',ftpPath:'+filepath );
...
function download(res , options){
    ...
    useLog.log('ftp download is start ',ftpPath);
    try{
        ftp.get(ftpPath,function(err,socket){
            if(socket){
                var count = 0;
                var isEnd = false,writeEnd = false;
                useLog.log('ftp socket start');
                socket.on('data',function(d){
                    count ++;
                    if(writeEnd)return;
                    useLog.log('WriteStream write start');
                    try{
                        if(WriteStream.write(d,function(){
                                useLog.log('WriteStream write end one');
                                //socket.resume();
                                count--;
                                if(!count && isEnd){
                                    socketEnd();
                ...
                function socketEnd(){
                    writeEnd = true;
                    WriteStream.end();
                    useLog.log('WriteStream write end all');
                ...
                socket.on('close',function(d){
                    useLog.log('WriteStream socket close');
                    isEnd = true;
                    if(!count){
                        socketEnd();
                    }
...
function rename(options){
    ...
    useLog.log('start ftp rename.' + startUrl + ' ' + endUrl);
...
function delFile(options){
    ...
    useLog.log('ftp delete is start.' + filename);
